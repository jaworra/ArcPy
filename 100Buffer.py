...
for lyr in arcpy.mapping.ListTableViews(mxd):
    ...
    lyr = "Seg_events"
    SaveLoc = "G:/Geospatial/GIS/JohnW/QRN/CaseStudy1/QLD_Development_Code_Noise_Contours/JW_Workings/20130319/Deliverables/"
    ...
    arcpy.CopyFeatures_management(ProjectedFeat,SaveLoc+tbl+"_SegmentedLineCOPY.shp") 
    arcpy.DeleteField_management(SaveLoc+tbl+"_SegmentedLineCOPY.shp",["F2","F4","F6","F13","F20","LOC_ERROR", "U85dbV","U80dbV","U75dbV","U70dbV","U150mV","U250mV","D85dbV","D80dbV","D75dbV","D70dbV","D150mV","D250mV",])

    #Rewrite any records > 250 to 250
    inputFC = tbl+"_SegmentedLineCOPY"   
    colCat1 = "X85"     
    colCat2 = "X80"
    colCat3 = "X75"
    colCat4 = "X70"
    ...
    #Segment line based on laga   ->  Time consuming process
    ...
    arcpy.DeleteField_management("MergeResultsSegLGA_Attribute",["NAME","NUMBER0","REGIONNAME","REGIONCODE","REGION_","Shape_Leng","Join_Count","TARGET_FID","Shape_Le_1"])
    arcpy.CopyFeatures_management("MergeResultsSegLGA_Attribute",SaveLoc+tbl+"_SegmentedLine.shp")
    arcpy.Delete_management(SaveLoc+tbl+"_SegmentedLineCOPY.shp")
...
#Segment line, base on coordinate system.....
rows = arcpy.UpdateCursor('StateWide_MetrageRoads')
...
for row in rows:
    feat = row.Shape
    print feat.length
#Change the last row of each record to reflec the end of the line......

# run Split Line at Vertices
result = arcpy.SplitLineAtVertices_management(orig_fc, r"C:\temp\my.gdb\lines_Split")
in_fc = result.getOutput(0)
...
try:
    ...
    first_row = rows.next()
    first_feat = first_row.getValue(shape_field_name)

   for row in rows:  # second line onward

        curr_feat = row.getValue(shape_field_name)
        
        if first_feat.contains(curr_feat) or curr_feat.contains(first_feat):
            if first_feat.length >= curr_feat.length:
                print 'delete', curr_feat  #do your stuff here.
            else:
                print 'delete', first_feat
            
        first_feat = curr_feat'

#Write a catch method to be included in 
...
Incorret Buffer polygon
--------------
Main Line - > missing 80km to end of line (12km to Ipswich boarder)
North Coast - > missing 1 km to intersection line GD (30km to Gympie boarder)

Gold Coast -> missing 40km to end of line
Hamilton Line -> missing 600m to end of line
Ferny Gorve -> missing 400m to the end of line 


Correct Buffer polygon
--------------
Shorncliffe 
Yeerongpilly 
Fisherman Island 
