...
def createNoiseContour(ProjectedFeat):
    
    #Left Analysis
    AnalysisSide = "LEFT"
    OutFileDir = AnalysisSide + "_Buffer"
    OutFileDis = AnalysisSide + "_Disolve"
    OutFileClip = AnalysisSide

    DistField = "U70dbV"
    SideType = "FULL"
    EndType = "ROUND"
    DisType = "NONE"
    featureClasses = ProjectedFeat

    MaxBuffer =  "150 meter"
    out_feature_class = "LeftFeat"
    xy_tolerance = ""

    for featureClass in featureClasses:
        arcpy.Buffer_analysis(output_features_class, OutFileDir ,DistField, SideType, EndType, DisType, )

    #Dissolve
    arcpy.Dissolve_management(OutFileDir, OutFileDis,"", "", "", "")    

    #Create polygon to clip
    arcpy.Buffer_analysis(rt, OutFileClip  ,MaxBuffer, AnalysisSide, EndType, DisType,) 

    # Execute Clip
    arcpy.Clip_analysis(OutFileDis, OutFileClip, out_feature_class, xy_tolerance)
    ...
    arcpy.Dissolve_management("MERGE", "Contour_70dBA","", "", "", "")
...
#Parse Varaiable here!!
#Segemented Line
createNoiseContour(ProjectedFeat) 
...
#Set local variables for Left Analysis
AnalysisSide = "LEFT"
OutFileDir = AnalysisSide + "_Buffer"
OutFileDis = AnalysisSide + "_Disolve"
OutFileClip = AnalysisSide
...
#Set local variables for Right Analysis
AnalysisSide = "RIGHT"
OutFileDir = AnalysisSide + "_Buffer"
OutFileDis = AnalysisSide + "_Disolve"
OutFileClip = AnalysisSide
...
for lyr in arcpy.mapping.ListLayers(mxd):
     if lyr.name != "SF_Metrageroads_CalibratedRoutes" and lyr.name != "SF_kmPosts" and lyr.name != "Contour_70dBA":# Layers to display
         lyr.visible = False
     if lyr.name == "Buffer_TTSF_LNR" or lyr.name == "Merge" or lyr.name == "Right" or lyr.name == "Left":
         arcpy.mapping.RemoveLayer(df, lyr)
arcpy.RefreshTOC()
arcpy.RefreshActiveView()    
...
for featureClass in featureClasses:
    arcpy.Buffer_analysis('TTSF_LNR' , OutFileDir ,DistField, SideType, EndType, DisType, )
